...
for t in range(tCases):
    ...
    bToWin = 0
    numShots = int(input())
    aRemaining = bRemaining = numShots
    shots = np.array(list(map(int, input())))
    for i in range(2*numShots):
        if i%2 == 0:  #shot played by a
            if shots[i] == 1:
                aScore += 1
                if aToWin == 0:
                    bToWin += 1
                    ...
                    aToWin -= 1
            aRemaining -= 1
            if aRemaining < aToWin:
                break
            if bRemaining < bToWin:
                break
        else:  #shot played by b
            if shots[i] == 1:
                bScore += 1
                if bToWin == 0:
                    aToWin += 1
